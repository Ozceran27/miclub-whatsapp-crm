-- Canonical onboarding progress and auditable opening balances.
BEGIN;
...
UPDATE miclub.clubs
SET base_currency_code = upper(settings->>'currency')
WHERE upper(coalesce(settings->>'currency','')) IN ('ARS','USD','BRL','EUR');
...
-- Relational state must never remain duplicated in clubs.settings.
UPDATE miclub.clubs
SET settings = coalesce(settings, '{}'::jsonb) - 'onboarding' - 'openingBalances' - 'opening_balances' - 'operationalBalances' - 'operational_balances' - 'currency',
    updated_at = now()
WHERE coalesce(settings, '{}'::jsonb) ?| ARRAY['onboarding','openingBalances','opening_balances','operationalBalances','operational_balances','currency'];
ALTER TABLE miclub.clubs DROP CONSTRAINT IF EXISTS clubs_settings_non_relational_check;
ALTER TABLE miclub.clubs ADD CONSTRAINT clubs_settings_non_relational_check CHECK (
  NOT coalesce(settings, '{}'::jsonb) ?| ARRAY['onboarding','openingBalances','opening_balances','operationalBalances','operational_balances','currency']
);
COMMENT ON COLUMN miclub.clubs.settings IS
  'Configuración tenant sin modelo relacional propio; onboarding, cuentas y saldos están prohibidos.';
...
BEGIN
  ...
  INSERT INTO miclub.financial_accounts(club_id,code,name,currency_code)
  SELECT p_club_id, seed.code, seed.name, CASE WHEN seed.code='USD_CASH' THEN 'USD' ELSE c.base_currency_code END
  FROM miclub.clubs c CROSS JOIN (VALUES ('CASH','Caja'),('BANK','Banco'),('USD_CASH','Caja USD')) seed(code,name)
  WHERE c.id=p_club_id ON CONFLICT (club_id,code) DO NOTHING;
...
-- Every declared opening balance is traceable to its ledger movement and account.
CREATE OR REPLACE VIEW miclub.v_opening_balance_reconciliation AS
SELECT b.club_id, b.id AS batch_id, b.revision, b.operation, b.status AS batch_status,
       b.idempotency_key, b.reconciliation_status, obm.movement_id, obm.reverses_movement_id,
       m.account_id, a.code AS account_code, a.currency_code, m.movement_type, m.amount,
       m.operational_status, m.reconciled_at,
       (m.club_id=b.club_id AND a.club_id=b.club_id AND m.movement_type='CAPITAL'
        AND m.operational_status='COMPLETADO' AND m.voided_at IS NULL) AS is_consistent
