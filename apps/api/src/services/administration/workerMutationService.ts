...
export const resolveWorkerInvitation = async (userId: string, token: string, decision: "accept" | "reject", request?: Pick<WorkerActor, "requestId" | "ip" | "userAgent">) => {
  ...
  const result = await withTransaction(async (db) => {
    ...
    const employee = (await db.query<Record<string, unknown>>(`insert into miclub.employees(club_id,person_id,user_id,membership_id,sector_id,status,payment_mode,monthly_fixed_amount,employment_start_date,position,notes,created_by,updated_by) values($1,$2,$3,$4,$5,'active',$6,$7,$8,$9,$10,$11,$11) returning *`, [invitation.club_id, person.id, userId, membership.id, data.sectorId ?? null, data.paymentMode, data.monthlyFixedAmount ?? null, data.employmentStartDate ?? null, data.role, data.notes ?? null, invitation.invited_by])).rows[0];
