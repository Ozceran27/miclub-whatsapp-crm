...
export const createWorker = async (actor: WorkerActor, body: unknown): Promise<Record<string, unknown>> => {
  ...
  return withTenantTransaction(actor.clubId, async (db) => {
    ...
    if (input.systemAccessEnabled === false) {
      ...
      if (input.role === "INSTRUCTOR") await db.query(`insert into miclub.instructors(club_id,person_id,display_name,status,is_active,notes) values($1,$2,$3,'activa',true,$4) on conflict (club_id,person_id) do update set display_name=excluded.display_name,status='activa',is_active=true,updated_at=now()`, [actor.clubId, person.id, `${input.firstName} ${input.lastName}`, input.notes ?? null]);
...
export const resolveWorkerInvitation = async (userId: string, token: string, decision: "accept" | "reject", request?: Pick<WorkerActor, "requestId" | "ip" | "userAgent">) => {
  ...
  const result = await withTransaction(async (db) => {
    ...
    if (data.role === "INSTRUCTOR") await db.query(`insert into miclub.instructors(club_id,person_id,display_name,status,is_active,notes) values($1,$2,$3,'activa',true,$4) on conflict (club_id,person_id) do update set display_name=excluded.display_name,status='activa',is_active=true,updated_at=now()`, [invitation.club_id, person.id, `${data.firstName} ${data.lastName}`, data.notes ?? null]);
...
export const updateWorker = async (actor: WorkerActor, id: string, body: unknown) => {
  ...
  return withTenantTransaction(actor.clubId, async (db) => {
    ...
    if (input.role === "INSTRUCTOR") await db.query(`insert into miclub.instructors(club_id,person_id,display_name,status,is_active,notes) values($1,$2,$3,'activa',true,$4) on conflict (club_id,person_id) do update set display_name=excluded.display_name,status='activa',is_active=true,notes=excluded.notes,updated_at=now()`, [actor.clubId, current.person_id, `${input.firstName} ${input.lastName}`, input.notes ?? null]);
    else if (instructor) await db.query(`update miclub.instructors set status='inactiva',is_active=false,updated_at=now() where club_id=$1 and id=$2`, [actor.clubId, instructor.id]);
...
export const archiveWorker = async (actor: WorkerActor, id: string) => {
  const pool = await getPostgresPool(); return withTenantTransaction(actor.clubId, async (db) => {
    ...
    if (instructor) await db.query(`update miclub.instructors set status='inactiva',is_active=false,updated_at=now() where club_id=$1 and id=$2`, [actor.clubId,instructor.id]);
