...
test("resuelve sólo grants vigentes con metadatos tenant-scoped", async () => {
  ...
  const executor: QueryExecutor = { query: <T>(_sql: string, values?: unknown[]) => {
    params = values;
    return Promise.resolve({ rows: [{ enabled: true, capability: grant.code, source: grant.source, effective_from: grant.effectiveFrom, effective_until: null, actor: grant.actor }] as T[] });
  } };
