...
export const DEBT_LIABILITY_CATEGORY_KEYS = [...DEBT_LIABILITY_CATEGORIES];
export const SERVICE_CATEGORY_KEYS = [...SERVICE_CATEGORIES];
export const getOperatingCategories = (): readonly string[] =>
...
export type ExpenseTypeKey =
  | (typeof EXPENSE_TYPE_KEYS)[number]
  | "UNCLASSIFIED";
export const EXPENSE_TYPE_LABELS: Record<
  Exclude<ExpenseTypeKey, "UNCLASSIFIED">,
  string
> = {
...
const classificationFromCode = (
  value: unknown,
): CategoryClassification | null => {
  const code = normalizeCategoryName(value);
  if ((OPERATING_CATEGORIES as readonly string[]).includes(code))
    return "OPERATIONAL";
  if ((NON_OPERATING_EXPENSE_CATEGORIES as readonly string[]).includes(code))
    return "NON_OPERATIONAL";
  if ((TAX_CATEGORIES as readonly string[]).includes(code)) return "TAX";
  if ((SERVICE_CATEGORIES as readonly string[]).includes(code))
    return "SERVICE";
  if ((DEBT_LIABILITY_CATEGORIES as readonly string[]).includes(code))
    return "LIABILITY";
  return null;
};
...
export type RollingInterannualMonth = {
  year: number;
  month: number;
  key: string;
  label: string;
...
export const getRollingInterannualMonthWindow = (reference = new Date()) => {
  ...
  for (let offset = 0; offset <= 12; offset += 1) {
    ...
    const label = shortFormatter
      .format(date)
      .replace(/\.$/, "")
      .replace(/^./, (char) => char.toLocaleUpperCase("es-AR"));
    const fullLabel = fullFormatter
      .format(date)
      .replace(/^./, (char) => char.toLocaleUpperCase("es-AR"));
    months.push({
      year: part.year,
...
export const isOperatingCategory = (value: unknown): boolean =>
  classificationFromCode(value) === "OPERATIONAL";

export const isNonOperatingExpenseCategory = (value: unknown): boolean =>
  classificationFromCode(value) === "NON_OPERATIONAL";

export const isServiceCategory = (value: unknown): boolean =>
  classificationFromCode(value) === "SERVICE";

export const isTaxCategory = (value: unknown): boolean =>
  classificationFromCode(value) === "TAX";

export const isDebtCategory = (value: unknown): boolean =>
  classificationFromCode(value) === "LIABILITY";

export const normalizeMovementStatus = (value: unknown): string =>
...
export const isCompletedMovement = (
  movement: SectorProfitabilityMovement,
): boolean =>
  isCompletedMovementStatus(
    movement.operational_status ??
      movement.operationalStatus ??
      movement.estado,
  );

export const normalizeMovementType = (value: unknown): string =>
  normalizeCategoryName(value);

export const isIncomeMovement = (
  movement: SectorProfitabilityMovement,
): boolean =>
...
export const isExpenseMovement = (
  movement: SectorProfitabilityMovement,
): boolean =>
  normalizeMovementType(
    movement.movement_type ?? movement.movementType ?? movement.tipo,
...
export const calculateCategoryBalance = (
  movements: SectorProfitabilityMovement[],
  categories: readonly string[],
) => {
  const categoryKeys = new Set(
    categories.map((category) => normalizeCategoryName(category)),
  );
  let income = 0;
  let expenses = 0;
  ...
  for (const movement of movements) {
    const category =
      movement.category ?? movement.categoryName ?? movement.categoria;
    if (
      !isCompletedMovement(movement) ||
      !categoryKeys.has(normalizeCategoryName(category))
    )
      continue;
...
export const calculateOperatingProfitability = (
  movements: SectorProfitabilityMovement[],
  options: { sector?: unknown } = {},
) => {
  const sectorFilter =
    options.sector == null ? null : normalizeCategoryName(options.sector);
  let income = 0;
  let expenses = 0;
  let movementsCount = 0;
  for (const movement of movements) {
    const classification = normalizeCategoryName(
      movement.categoryClassification ?? movement.classification,
    );
    const categoryCode =
  ...
  return {
    income: roundMoney(income),
    expenses: roundMoney(expenses),
    expense: roundMoney(expenses),
    profitability: roundMoney(income - expenses),
    movementsCount,
  };
};

export const calculateOperatingProfitabilityBySector = (
  movements: SectorProfitabilityMovement[],
  options: { sector?: unknown } = {},
) => {
...
export const calculateSectorProfitability = (
  ...
  return Array.from(sectors.values()).sort(
    (a, b) => b.balance - a.balance || b.income - a.income,
  );
};
...
export const getRolling30DayWindows = (reference = new Date()) => {
  ...
  return {
    ...
    current: {
      from: currentStart,
      to: currentEnd,
      labelFrom: formatArgentinaLabel(currentStart),
      labelTo: formatArgentinaLabel(addDays(currentEnd, -1)),
      dateFrom: formatArgentinaDate(currentStart),
      dateTo: formatArgentinaDate(addDays(currentEnd, -1)),
    },
    previous: {
      from: previousStart,
      to: currentStart,
      labelFrom: formatArgentinaLabel(previousStart),
      labelTo: formatArgentinaLabel(addDays(currentStart, -1)),
      dateFrom: formatArgentinaDate(previousStart),
      dateTo: formatArgentinaDate(addDays(currentStart, -1)),
    },
    timezone: ARGENTINA_TIME_ZONE,
  };
};
...
export const getLastCompleteMonthWindows = (reference = new Date()) => {
  ...
  const currentStart =
    month === 1
      ? argentinaDayStart(year - 1, 12, 1)
      : argentinaDayStart(year, month - 1, 1);
  const previousStart =
    month <= 2
  ...
  return {
    ...
    currentLabel: label(currentStart),
    previousLabel: label(previousStart),
  };
};
...
export const calculateVariation = (
  ...
  const positiveImpact = inverseImpact
    ? direction === "down"
    : direction === "up";
  const impact: VariationImpact =
    direction === "stable"
      ? "neutral"
      : positiveImpact
        ? "favorable"
...
/** Calculates a balance from persisted catalog classification; display labels are deliberately ignored. */
export const calculateClassifiedBalance = (movements: SectorProfitabilityMovement[], classification: CategoryClassification) => {
  let income = 0;
  let expenses = 0;
  let movementsCount = 0;
  for (const movement of movements) {
    if (!isCompletedMovement(movement) || normalizeCategoryName(movement.categoryClassification ?? movement.classification) !== classification) continue;
    const amount = Math.abs(normalizeAmount(movement.amount ?? movement.monto));
    if (isIncomeMovement(movement)) income += amount;
    else if (isExpenseMovement(movement)) expenses += amount;
    else continue;
