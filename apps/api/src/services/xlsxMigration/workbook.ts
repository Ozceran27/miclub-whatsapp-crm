...
const enrollmentStatus=(value:unknown)=>{const key=String(value??"").trim().toLowerCase().replace(/\s+/g,"_"); return ({activo:"al_dia","al_día":"al_dia",nuevo:"nuevo_inscripto",moroso:"adeudando",baja:"abandonado",cancelado:"cancelado"} as Record<string,string>)[key]??"otro";};
    ...
    const persistedWrites=await deps.transaction(input.actor.clubId,async(db)=>{
      ...
      for(const row of input.rows){
        ...
        if(row.sheet==="ADMINISTRACIÓN"){
          ...
          const result=await db.query<{id:string}>(`insert into miclub.movements(club_id,sequence_number,external_id,movement_date,movement_type,category_id,sector_id,concept,counterparty_text,amount,taxes,payment_method_id,financial_status,operational_status,source,source_payload) values($1,miclub.next_tenant_sequence($1,'movement'),$2,$3,$4::miclub.movement_type,$5,$6,$7,$8,$9,$10,$11,'otro',$12::miclub.movement_status,'xlsx',$13) returning id`,[input.actor.clubId,resolved.externalReference??`xlsx:${resolved.rowFingerprint}`,row.values.date,movementType(row.values.type),resolved.categoryId,resolved.sectorId,row.values.concept,row.values.counterparty,row.values.amount,row.values.taxes??0,resolved.paymentMethodId,movementStatus(row.values.status),JSON.stringify({sheet:row.sheet,rowNumber:row.rowNumber,fingerprint:resolved.rowFingerprint})]); entityId=result.rows[0].id;
        } else {
          if(!resolved.personId||!resolved.activityId) throw error("UNRESOLVED_ROW",`Persona o actividad no resuelta en fila ${row.rowNumber}`);
          const result=await db.query<{id:string}>(`insert into miclub.enrollments(club_id,sequence_number,external_id,person_id,activity_id,fee_amount,status,due_date,enrollment_date,source,notes) values($1,miclub.next_tenant_sequence($1,'enrollment'),$2,$3,$4,$5,$6::miclub.enrollment_status,$7,$8,'xlsx',$9) returning id`,[input.actor.clubId,resolved.externalReference??`xlsx:${resolved.rowFingerprint}`,resolved.personId,resolved.activityId,row.values.fee,enrollmentStatus(row.values.status),row.values.expiresOn,row.values.date,JSON.stringify({sheet:row.sheet,rowNumber:row.rowNumber,fingerprint:resolved.rowFingerprint})]); entityId=result.rows[0].id;
        }
