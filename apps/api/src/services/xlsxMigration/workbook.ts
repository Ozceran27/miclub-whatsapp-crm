...
async function insertErrors(db:QueryExecutor,input:WorkbookInput,batchId:string){
  for(const item of input.errors) await db.query(`insert into miclub.import_errors(batch_id,club_id,source,row_number,severity,message,details,error_code,sheet,entity_type,field,value_normalized) values($1,$2,'xlsx_import',$3,$4,$5,$6,$7,$8,$9,$10,$11)`,[batchId,input.actor.clubId,item.row_number??null,item.severity,item.message,JSON.stringify({value_original:item.value_original??null}),item.error_code,item.sheet??null,item.entity_type??null,item.field??null,item.value_normalized??null]);
}
