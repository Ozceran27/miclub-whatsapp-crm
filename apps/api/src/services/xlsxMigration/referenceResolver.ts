...
export async function loadReferenceCatalog(clubId:string):Promise<ReferenceCatalog> {
  return withTenantTransaction(clubId,async(db)=>{
    ...
    return {sectors:sectors.rows,activities:activities.rows.map((a)=>({id:a.id,name:a.name,code:a.code,sectorId:a.sector_id,instructorId:a.instructor_id,modality:a.modality})),instructors:instructors.rows,categories:categories.rows,paymentMethods:paymentMethods.rows,people:people.rows,accounts:accounts.rows};
  });
}
