import { ROLE_DEFAULT_PERMISSIONS, type ClubRegistrationDto } from "@miclub/shared";
...
export async function provisionClub(
  ...
  const club = await client.query<{ id: string }>(`
    ...
    values ($1 || '-' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 8), $2,
      ...
      '{"configuration":{},"onboarding":{"status":"NOT_STARTED"}}'::jsonb)
  ...
  const role = await client.query<{ id: string }>(`
    ...
    values ($1, 'DIRECTOR', 'Director', 'Control total del club') returning id`, [clubId]);
  ...
  [user.rows[0].id, clubId, role.rows[0].id, [...ROLE_DEFAULT_PERMISSIONS.DIRECTOR]]);
