...
export const getPostgresClubFinanceSummary =
  async (clubId: string): Promise<ClubOperationsSummary> => {
    ...
    return {
      ...
      liquidity,
      valuationStatus: pickString(row, ["valuation_status"], "INCOMPLETE_EXCHANGE_RATE") as "COMPLETE" | "INCOMPLETE_EXCHANGE_RATE",
      unvaluedAccountCount: pickNumber(row, ["unvalued_account_count"]),
      missingPairs: Array.isArray(row.missing_pairs) ? row.missing_pairs as Array<{baseCurrencyCode:string;quoteCurrencyCode:string}> : [],
      valuationMode: "LIVE_RECALCULATED",
      cash: pickNumber(row, ["cash"]),
      bank: pickNumber(row, ["bank", "bank_balance"]),
      dollars: pickNumber(row, ["dollars", "usd"]),
      dollarsConverted: pickNumber(row, ["dollars_converted"]),
      presentationCurrencyCode: pickString(row, ["presentation_currency_code"], "ARS"),
      appliedRate: pickNullableNumber(row, ["applied_rate"]),
      rateDate: toStringValue(pick(row, ["rate_date"])) ?? null,
      rateSource: toStringValue(pick(row, ["rate_source"])) ?? null,
      rateDirection: (toStringValue(pick(row, ["rate_direction"])) as "DIRECT" | "INVERSE" | "IDENTITY" | undefined) ?? null,
      pendingIncome,
      pendingExpenses,
      pendingNetBalance,
...
export const emptyPostgresClubFinanceSummary = (): ClubOperationsSummary => ({
  ...
  liquidity: 0,
  valuationStatus: "INCOMPLETE_EXCHANGE_RATE",
