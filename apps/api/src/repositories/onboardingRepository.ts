...
type CompletionContext = { db:QueryExecutor; actor:OnboardingActor; draft:OnboardingDraft; sectorMap:Map<string,string>; workerMap:Map<string,string>; employeeMap:Map<string,string>; workerPersonMap:Map<string,string> };
...
export const completeOnboardingDraft=async(actor:OnboardingActor,draft:OnboardingDraft):Promise<CompleteOnboardingResult>=>withTenantTransaction(actor.clubId,async db=>{
 const ctx:CompletionContext={db,actor,draft,sectorMap:new Map(),workerMap:new Map(),employeeMap:new Map(),workerPersonMap:new Map()};
 ...
 if(draft.activities.length && (!await hasActivityResponsibleEmployee(db) || !await hasCanonicalActivityMutationGuard(db)))throw Object.assign(new Error('La migración canónica de responsables de actividades todavía no fue aplicada. Ejecute el SQL versionado antes de finalizar actividades.'),{code:'ONBOARDING_SCHEMA_UNAVAILABLE'});
