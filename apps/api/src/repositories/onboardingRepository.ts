...
type CompletionContext = { db:QueryExecutor; actor:OnboardingActor; draft:OnboardingDraft; sectorMap:Map<string,string>; workerMap:Map<string,string> };
const completionResultFromRow=(value:unknown):CompleteOnboardingResult=>value as CompleteOnboardingResult;
...
async function finalizePlan(ctx:CompletionContext,planCode:string){const {db,actor}=ctx;const billing=billingService.prepareOnboardingSelection(planCode as Parameters<typeof billingService.prepareOnboardingSelection>[0]);const current=(await db.query<{id:string;plan_code:string;billing_status:string}>(`select id::text,plan_code,billing_status from miclub.club_subscriptions where club_id=$1 and effective_from<=now() and (effective_until is null or effective_until>now()) order by effective_from desc,id desc limit 1 for update`,[actor.clubId])).rows[0];if(current?.plan_code===planCode&&current.billing_status===billing.status)return;await db.query(`update miclub.club_subscriptions set effective_until=now() where club_id=$1 and effective_from<now() and (effective_until is null or effective_until>now())`,[actor.clubId]);await db.query(`insert into miclub.club_subscriptions(club_id,plan_code,effective_from,billing_status) values($1,$2,now(),$3)`,[actor.clubId,planCode,billing.status]);}
...
export const completeOnboardingDraft=async(actor:OnboardingActor,draft:OnboardingDraft):Promise<CompleteOnboardingResult>=>withTenantTransaction(actor.clubId,async db=>{
 const ctx:CompletionContext={db,actor,draft,sectorMap:new Map(),workerMap:new Map()};
 ...
 const completed=[1,2,...(sectorIds.length?[3]:[]),...(draft.workers.length?[4]:[]),...(draft.activities.length?[5]:[]),...(draft.pendingImport?[6]:[]),7];const skipped=[...(sectorIds.length?[]:[3]),...(draft.workers.length?[]:[4]),...(draft.activities.length?[]:[5]),...(draft.pendingImport?[]:[6])];
 await db.query(`update miclub.club_onboarding set status='COMPLETED',current_step=7,completed_steps=$2,skipped_steps=$3,started_at=coalesce(started_at,now()),completed_at=coalesce(completed_at,now()),updated_at=now() where club_id=$1`,[actor.clubId,completed,skipped]);const state=map((await db.query<Row>(select,[actor.clubId])).rows[0]);await audit(actor,'onboarding.complete',before,state,db);const result={state,created:{openingBalanceBatchId,sectorIds,workerIds,activityIds}};await db.query(`update miclub.onboarding_operations set result=$3::jsonb,completed_at=now() where club_id=$1 and operation=$2`,[actor.clubId,'COMPLETE_ONBOARDING',JSON.stringify(result)]);return result;
},await getPostgresPool());
