...
async function finalizeFileAssociations(ctx:CompletionContext){
 if(ctx.draft.pendingImport)await ctx.db.query(`update miclub.import_batches set onboarding_completion_key=$3 where id=$1 and club_id=$2`,[ctx.draft.pendingImport.batchId,ctx.actor.clubId,ctx.draft.idempotencyKey]);
