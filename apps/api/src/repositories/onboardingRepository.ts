...
export type OnboardingActor = { userId:string; personId:string; membershipId:string; clubId:string; requestId?:string; ip?:string; userAgent?:string };
...
export const saveOpeningBalances=async(actor:OnboardingActor,input:OpeningBalancesRequest)=>withTenantTransaction(actor.clubId,async db=>{
 const batchId=await replaceOpeningBalances(db,actor,input.currency,input.cash,input.bank,input.usdCash,input.idempotencyKey);
 await auditService.sensitiveChange({action:"onboarding.opening_balances",result:"success",userId:actor.userId,membershipId:actor.membershipId,clubId:actor.clubId,entityType:"opening_balance_batch",entityId:batchId,requestId:actor.requestId,ip:actor.ip,userAgent:actor.userAgent,newData:{currency:input.currency,cash:input.cash,bank:input.bank,usdCash:input.usdCash}},db);
 return {batchId};
},await getPostgresPool());


type CompletionContext = { db:QueryExecutor; actor:OnboardingActor; draft:OnboardingDraft; sectorMap:Map<string,string>; workerMap:Map<string,string>; employeeMap:Map<string,string>; workerPersonMap:Map<string,string>; activityMutationActorId:string|null };
...
async function finalizePlan(ctx:CompletionContext,planCode:string){const {db,actor}=ctx;const billing=billingService.prepareOnboardingSelection(planCode as Parameters<typeof billingService.prepareOnboardingSelection>[0]);const current=(await db.query<{id:string;plan_code:string;billing_status:string;selection_mode:string;selection_source:string}>(`select id::text,plan_code,billing_status,selection_mode,selection_source from miclub.club_subscriptions where club_id=$1 and effective_from<=now() and (effective_until is null or effective_until>now()) order by effective_from desc,id desc limit 1 for update`,[actor.clubId])).rows[0];if(current?.plan_code===planCode&&current.billing_status===billing.status&&current.selection_mode===billing.mode&&current.selection_source===billing.source)return;if(current)await db.query(`update miclub.club_subscriptions set effective_until=greatest(clock_timestamp(),effective_from+interval '1 microsecond') where club_id=$1 and id=$2`,[actor.clubId,current.id]);await db.query(`insert into miclub.club_subscriptions(club_id,plan_code,effective_from,billing_status,selection_mode,selection_source) values($1,$2,clock_timestamp(),$3,$4,$5)`,[actor.clubId,planCode,billing.status,billing.mode,billing.source]);}
async function finalizeBalances(ctx:CompletionContext){const {db,actor,draft}=ctx;return replaceOpeningBalances(db,actor,draft.openingBalances.currency,draft.openingBalances.cash,draft.openingBalances.bank,draft.openingBalances.usdCash,draft.idempotencyKey);}
...
export const completeOnboardingDraft=async(actor:OnboardingActor,draft:OnboardingDraft):Promise<CompleteOnboardingResult>=>withTenantTransaction(actor.clubId,async db=>{
 const ctx:CompletionContext={db,actor,draft,sectorMap:new Map(),workerMap:new Map(),employeeMap:new Map(),workerPersonMap:new Map(),activityMutationActorId:null};
 ...
 if(draft.activities.length){
  if(!await hasActivityResponsibleEmployee(db) || !await hasCanonicalActivityMutationGuard(db))throw Object.assign(new Error('La migración canónica de responsables de actividades todavía no fue aplicada. Ejecute el SQL versionado antes de finalizar actividades.'),{code:'ONBOARDING_SCHEMA_UNAVAILABLE'});
  ctx.activityMutationActorId=await resolveActivityMutationActorId(db,actor);
  if(!ctx.activityMutationActorId)throw Object.assign(new Error('La identidad de auditoría de actividades no coincide con el esquema instalado.'),{code:'ONBOARDING_SCHEMA_UNAVAILABLE'});
 }
