...
async function finalizeActivities(ctx:CompletionContext){
  ...
  for(const item of ctx.draft.activities){
    ...
    await ctx.db.query(`insert into miclub.activity_price_terms(club_id,activity_id,enrollment_price,fee_price,fee_frequency,currency_code,effective_from,created_by,updated_by)
