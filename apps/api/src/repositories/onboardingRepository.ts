...
async function finalizeActivities(ctx:CompletionContext){const ids:string[]=[];for(const item of ctx.draft.activities){const responsibleClientId=item.responsibleWorkerClientId??item.instructorClientId;if(!responsibleClientId)throw Object.assign(new Error('La actividad requiere un trabajador responsable.'),{code:'ONBOARDING_ACTIVITY_RESPONSIBLE_REQUIRED'});if(!ctx.activityMutationActorId)throw Object.assign(new Error('La identidad de auditoría de actividades no coincide con el esquema instalado.'),{code:'ONBOARDING_SCHEMA_UNAVAILABLE'});const row=(await ctx.db.query<{id:string}>(`insert into miclub.activities(club_id,sector_id,instructor_id,responsible_employee_id,name,color,icon_key,club_commission_percent,instructor_commission_percent,generates_enrollments,status,updated_by) values($1,$2,$3,$4,$5,$6,$7,$8,0,$9,$10,$11) returning id::text`,[ctx.actor.clubId,ctx.sectorMap.get(item.sectorClientId),ctx.workerMap.get(responsibleClientId)??null,ctx.employeeMap.get(responsibleClientId),item.name,item.color,item.iconKey,item.settlementMode==='VARIABLE'?item.clubSharePercentage:0,item.generatesEnrollments,storedEntityStatus(item.status),ctx.activityMutationActorId])).rows[0];ids.push(row.id);await ctx.db.query(`insert into miclub.activity_terms(club_id,activity_id,mode,fixed_club_fee,fixed_fee_frequency,currency_code,club_share_percentage,responsible_person_id,effective_from,created_by,updated_by) values($1,$2,$3,$4,$5,$6,$7,$8,current_date,$9,$9)`,[ctx.actor.clubId,row.id,item.settlementMode,item.fixedClubFee,item.fixedFeeFrequency,item.currencyCode,item.clubSharePercentage,ctx.workerPersonMap.get(responsibleClientId),ctx.actor.userId]);}return ids;}
