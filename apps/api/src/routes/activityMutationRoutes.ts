...
const parseInput = (body: Record<string, unknown>, res: Response, requireSettlement: boolean): ActivityInput | null => {
  ...
  if (body.status !== undefined && (typeof body.status !== "string" || !["active", "inactive"].includes(body.status))) { fail(res, 400, "VALIDATION_ERROR", "status debe ser active o inactive."); return null; }
  if (!settlement) {
    const contract = body as unknown as ActivityMutationContract;
    return { ...contract, color, settlement: undefined, managerPersonId: null, clubCommissionPercent: null, name: body.name.trim() };
  }
...
router.post("/activities", requirePermission(PERMISSIONS.ACTIVITIES_CREATE), requireSectorAccess((req) => req.body?.sectorId), asyncHandler(async (req, res) => { const input = parseInput(req.body, res, true); if (input) return respond(res, await createActivity(actor(req), input)); }));
router.patch("/activities/:id", requirePermission(PERMISSIONS.ACTIVITIES_EDIT), asyncHandler(async (req, res) => { const id = String(req.params.id); if (!UUID.test(id)) return fail(res, 400, "VALIDATION_ERROR", "id inválido."); const expected = version(req.body, res); const input = parseInput(req.body, res, false); if (expected && input) return respond(res, await updateActivity(actor(req), id, expected, input)); }));
