...
export const isCompleteOnboardingRequest = (body: unknown): body is CompleteOnboardingRequest => {
  ...
  if (!draft.workers.every((worker) =>
    ...
    && ["TRABAJADOR", "INSTRUCTOR", "DIRECTOR"].includes(String(worker.role))
  ...
  const workerIds = new Set(draft.workers.map((worker) => worker.clientId));
  if (!draft.activities.every((activity) =>
    !Object.keys(activity).some((key) => !["clientId","sectorClientId","responsibleWorkerClientId","instructorClientId","name","iconKey","color","status","generatesEnrollments","settlementMode","fixedClubFee","fixedFeeFrequency","currencyCode","clubSharePercentage","pricing","schedules"].includes(key))
