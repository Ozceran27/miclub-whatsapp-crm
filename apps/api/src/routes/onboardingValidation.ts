import { SUPPORTED_OPERATIONAL_CURRENCIES, type CompleteOnboardingRequest, type OpeningBalancesRequest } from "@miclub/shared";
...
export const isCompleteOnboardingRequest=(body:unknown):body is CompleteOnboardingRequest=>{
 ...
 if(!d.sectors.every(x=>typeof x.name==='string'&&x.name.trim().length>0&&x.name.length<=120&&/^#[0-9a-f]{6}$/i.test(x.color)&&['active','inactive','under_repair'].includes(x.status)))return false;
