...
import { financeTransaction, financialError, readFinancialCircuit, recordResponsiblePayment, resolveTerm, reviewSettlement } from '../services/financialCircuitService.js';
...
import { approveStartup, previewStartup, reconcileMovement } from '../services/financialStartupService.js';
...
router.get('/finance/workbench', requirePermission(PERMISSIONS.FINANCE_READ), asyncHandler(async (req, res) => {
  ...
  res.json(await withTenantTransaction(req.auth!.clubId, async db => {
    ...
    const startup = (await db.query('select mode,cutoff_date::text "cutoffDate",revision,status,expected_balances preview,approved_snapshot from miclub.finance_startups where club_id=$1', [club])).rows[0] ?? null;
    return { movements, categories, activities, sectors: sectorRows, methods, receivables, enrollments, startup };
  }));
}));
...
router.get('/finance/history/:entity/:id', requirePermission(PERMISSIONS.FINANCE_READ), asyncHandler(async (req, res) => {
...
router.post('/finance/movements', requirePermission(PERMISSIONS.MOVEMENTS_CREATE), asyncHandler(async (req, res) => {
  ...
  res.status(201).json(await run(req, db => createFinancialMovement(db, req.auth!, cash(b.movement), b.applications as Parameters<typeof createFinancialMovement>[3])));
}));
router.post('/finance/movements/:id/correct', requirePermission(PERMISSIONS.FINANCE_CORRECT), asyncHandler(async (req, res) => {
  ...
  res.json(await run(req, db => correctFinancialMovement(db, req.auth!, uuid(req.params.id), revision(b.revision), cash(b.movement), b.applications as Parameters<typeof correctFinancialMovement>[5])));
}));
