...
router.get("/tasks",requirePermission(PERMISSIONS.TASKS_VIEW),asyncHandler(async(req,res)=>res.json({items:await listTasks(req.auth!.clubId)})));
router.post("/tasks",requirePermission(PERMISSIONS.TASKS_CREATE),asyncHandler(async(req,res)=>{const input=parse(req.body,false,res);if(input)return respond(res,await createTask(actor(req),input as TaskInput))}));
router.patch("/tasks/:id",requirePermission(PERMISSIONS.TASKS_EDIT),asyncHandler(async(req,res)=>{const id=String(req.params.id);if(!UUID.test(id))return fail(res,400,"VALIDATION_ERROR","id inválido.");const v=version(req.body,res),input=parse(req.body,true,res);if(v&&input)return respond(res,await updateTask(actor(req),id,v,input))}));
router.patch("/tasks/:id/status",requirePermission(PERMISSIONS.TASKS_EDIT),asyncHandler(async(req,res)=>{const id=String(req.params.id);if(!UUID.test(id)||Object.keys(req.body).some(k=>!["updatedAt","status"].includes(k))||!TASK_STATUSES.includes(req.body.status))return fail(res,400,"VALIDATION_ERROR","Estado o id inválido.");const v=version(req.body,res);if(v)return respond(res,await setTaskStatus(actor(req),id,v,req.body.status))}));
router.post("/tasks/:id/archive",requirePermission(PERMISSIONS.TASKS_EDIT),asyncHandler(async(req,res)=>{const id=String(req.params.id);if(!UUID.test(id)||Object.keys(req.body).some(k=>k!=="updatedAt"))return fail(res,400,"VALIDATION_ERROR","Solicitud inválida.");const v=version(req.body,res);if(v)return respond(res,await archiveTask(actor(req),id,v))}));
