import { SUPPORTED_OPERATIONAL_CURRENCIES, type OnboardingDestination, type OnboardingDraft, type OnboardingStep, type OnboardingStepOutcome } from '@miclub/shared';
...
type Props = { step:OnboardingStep; direction:'forward'|'backward'; draft:OnboardingDraft; updateDraft:<K extends keyof OnboardingDraft>(key:K,value:OnboardingDraft[K])=>void; migrationAvailable: boolean; pending: boolean; success: boolean; destination:OnboardingDestination; error: string; onNext:(outcome?:OnboardingStepOutcome)=>void; onBack:()=>void; onComplete: () => void; onContinue:()=>void };
...
export function OnboardingDialog({ step,direction,draft,updateDraft,migrationAvailable, pending, success, destination, error, onNext,onBack,onComplete,onContinue }: Props) {
