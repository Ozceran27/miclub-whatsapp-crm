...
export function OnboardingDraftSummary({draft}:{draft:OnboardingDraft}) {
 ...
 const omitted=[customSectors===0?'Sectores':null,draft.workers.length===0?'Trabajadores':null,draft.activities.length===0?'Actividades':null,draft.pendingImport===null?'Plan / migración':null].filter(Boolean).join(', ');
 return <section className="onboarding-summary" aria-labelledby="onboarding-summary-title"><h3 id="onboarding-summary-title">Resumen de tu configuración</h3><div className="onboarding-summary__grid">
  <OnboardingSummaryCard label="Moneda y saldos" value={draft.openingBalances.currency} detail={`Caja ${money(draft.openingBalances.cash,draft.openingBalances.currency)} · Cuenta ${money(draft.openingBalances.bank,draft.openingBalances.currency)} · Dólares ${money(draft.openingBalances.usdCash,'USD')}`}/>
  <OnboardingSummaryCard label="Sectores" value={`${draft.sectors.length} en total`} detail={`${customSectors} personalizados · ${draft.sectors.length-customSectors} esenciales`}/>
  ...
  <OnboardingSummaryCard label="Plan / migración" value={draft.pendingImport?'Migración preparada':'Sin plan de migración elegido'}/><OnboardingSummaryCard label="Pasos omitidos" value={omitted||'Ninguno'} detail={omitted?'Podés completarlos luego desde Administración.':undefined}/>
 </div></section>;
}
