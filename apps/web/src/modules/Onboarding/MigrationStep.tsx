...
export function MigrationStep({selected,onSelect}:Props) {
 ...
 return <div className="migration-intro">
  <section className="migration-intro__plans" aria-labelledby="migration-plans-title">
   ...
   {plans&&<p className="onboarding-warning" role="note">{migrationEnabled?<><strong>El plan elegido concede Migración al completar el onboarding.</strong> Después verificaremos la capacidad efectiva y te ofreceremos “Ir a Migración” para cargar, validar con dry-run y aplicar el XLSX. En este sandbox no se solicita una tarjeta.</>:<><strong>Free completa el onboarding sin Migración.</strong> Al finalizar te ofreceremos “Ir al panel”; podés cambiar a un plan habilitado más adelante.</>}</p>}
  </section>
  <details className="migration-guide"><summary>1. Conocer el modelo XLSX</summary><div className="migration-intro__workbook"><h3>Tres hojas con referencias explícitas</h3><div className="migration-sheet-grid"><article><strong>ADMINISTRACIÓN</strong><p>Ingresos, egresos y capital con sus referencias.</p></article><article><strong>INSCRIPCIONES</strong><p>Personas, actividades, cuotas y estados.</p></article><article><strong>SALDOS_INICIALES</strong><p>Deudas históricas de alumnos, responsables, empleados y proveedores. Son distintas del dinero de Caja y Banco configurado antes; quedan pendientes de conciliación.</p></article></div><p><strong>Las referencias deben coincidir exactamente</strong> con los catálogos configurados en miClub.</p><button className="secondary-button" type="button" onClick={()=>void downloadTemplate()}>Descargar Modelo_Import_miClub.xlsx</button></div></details>
  <details className="migration-guide"><summary>2. Preparar la importación posterior</summary><section className="migration-intro__flow"><ol><li><span>1</span><div><strong>Completar onboarding</strong><p>Primero confirmá la configuración y el plan.</p></div></li><li><span>2</span><div><strong>Ir a Migración</strong><p>Cargá allí el XLSX; este paso no acepta archivos.</p></div></li><li><span>3</span><div><strong>Dry-run</strong><p>Revisá todos los errores.</p></div></li><li><span>4</span><div><strong>Confirmar</strong><p>Aplicá únicamente ese lote validado.</p></div></li></ol></section></details>
