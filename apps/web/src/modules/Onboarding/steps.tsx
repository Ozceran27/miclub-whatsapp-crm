...
export const getOnboardingSteps = (_migrationAvailable:boolean,draft:OnboardingDraft,update:<K extends keyof OnboardingDraft>(key:K,value:OnboardingDraft[K])=>void) => [
 {eyebrow:'Tu club empieza acá',icon:'🏟',title:'¡Te damos la bienvenida a miClub!',description:'En pocos minutos vas a preparar la base de gestión de tu institución.',body:<div className="onboarding-welcome"><OnboardingStepList items={welcomeSteps}/></div>},
 ...
 {eyebrow:'Espacios del club',icon:'⌂',title:'Organizá tus sectores',description:'Prepará las áreas nuevas del club.',body:<SectorDraftList items={draft.sectors} activities={draft.activities} onChange={v=>update('sectors',v)}/>},
 {eyebrow:'Tu equipo',icon:'♙',title:'Sumá a tus trabajadores',description:'Prepará las altas que se crearán al finalizar.',body:<WorkerDraftList items={draft.workers} activities={draft.activities} currency={draft.openingBalances.currency} onChange={v=>update('workers',v)}/>},
 {eyebrow:'Propuesta deportiva',icon:'◆',title:'Configurá las actividades',description:'Prepará el catálogo inicial.',body:<ActivityDraftList items={draft.activities} sectors={draft.sectors} workers={draft.workers} currency={draft.openingBalances.currency} onChange={v=>update('activities',v)}/>},
 {eyebrow:'Traé tu información',icon:'⇧',title:'Elegí tu plan y prepará la migración',description:'La importación se realiza después del onboarding desde el panel.',body:<MigrationStep selected={draft.selectedPlanCode} onSelect={value=>update('selectedPlanCode',value)}/>},
