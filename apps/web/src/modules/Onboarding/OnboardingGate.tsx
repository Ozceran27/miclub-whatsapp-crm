import { ONBOARDING_DRAFT_CONTRACT_VERSION, PROVISIONED_ONBOARDING_SECTORS, type OnboardingDraft, type OnboardingState, type OnboardingStep, type OnboardingStepOutcome } from '@miclub/shared';
...
export function OnboardingGate({ children }: { children: ReactNode }) {
  ...
  const complete = async () => { if(loading)return; setLoading(true); setError(''); try { const result = await completeOnboarding(draft); setState({...result.state,status:'COMPLETED',shouldShow:true}); setCompletionConfirmed(true); invalidateTenantQueries(clubId); await loadHomeDashboardResources(); window.setTimeout(()=>navigate(result.state.migrationAvailable?'/app/migration':'/app', { replace: true }),1200); } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo completar el onboarding.'); } finally { setLoading(false); } };
