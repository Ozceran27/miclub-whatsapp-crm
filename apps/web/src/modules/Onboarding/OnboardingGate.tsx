...
export function OnboardingGate({ children }: { children: ReactNode }) {
  ...
  const advance = async (step: OnboardingStep) => { setLoading(true); setError(''); try { setState(await advanceOnboarding(step)); } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo guardar el avance.'); } finally { setLoading(false); } };
  ...
  return <>{children}{state?.shouldShow && <OnboardingDialog step={state.currentStep} pending={loading} error={error} onAdvance={(step) => void advance(step)} onComplete={() => void complete()} />}</>;
}
