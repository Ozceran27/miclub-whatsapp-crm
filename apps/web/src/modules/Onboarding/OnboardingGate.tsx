...
export function OnboardingGate({ children }: { children: ReactNode }) {
  ...
  const [,setStepOutcomes]=useState<Partial<Record<OnboardingStep,OnboardingStepOutcome>>>({});
  ...
  useEffect(() => { if (!state?.shouldShow) return; setVisibleStep(1); setDirection('forward'); setStepOutcomes({}); }, [clubId,state?.shouldShow]);
  ...
  const onNext=(outcome:OnboardingStepOutcome='COMPLETED')=>{setStepOutcomes(current=>({...current,[visibleStep]:outcome}));setDirection('forward');setVisibleStep(current=>Math.min(7,current+1) as OnboardingStep);};
