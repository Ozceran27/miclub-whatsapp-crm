...
const presentationByCode = {
  ARS: { name: 'Peso argentino', symbol: '$', locale: 'es-AR', flag: '🇦🇷' },
  USD: { name: 'Dólar estadounidense', symbol: 'US$', locale: 'en-US', flag: '🇺🇸' },
  BRL: { name: 'Real brasileño', symbol: 'R$', locale: 'pt-BR', flag: '🇧🇷' },
  EUR: { name: 'Euro', symbol: '€', locale: 'es-ES', flag: '🇪🇺' },
...
export const formatCurrencyLabel = (code: string): string => {
  ...
  return currency ? `${currency.flag} ${currency.name} (${currency.code})` : `Moneda desconocida (${code || '—'})`;
};
