...
export function SectorDraftList({ items, onChange }: { items: OnboardingSectorDraft[]; onChange: (items: OnboardingSectorDraft[]) => void }) {
  ...
  const save = (event: FormEvent<HTMLFormElement>) => {
    ...
    const base = { clientId: current?.clientId ?? crypto.randomUUID(), iconKey: field('iconKey'), code: current?.code ?? name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-'), name, color: field('color'), status: field('status') as OnboardingSectorDraft['status'], isSystem: false };
  ...
  return <section className="draft-editor"><h3>Sectores del club</h3><div className="draft-card-grid">
    ...
    <fieldset><legend>Modo de capacidad</legend><label><input type="radio" name="capacityMode" value="INCOME" checked={capacityMode === 'INCOME'} onChange={() => setCapacityMode('INCOME')}/> Ingresos</label><label><input type="radio" name="capacityMode" value="ENROLLMENTS" checked={capacityMode === 'ENROLLMENTS'} onChange={() => setCapacityMode('ENROLLMENTS')}/> Espacio Disponible</label></fieldset>
    {capacityMode === 'ENROLLMENTS' && <label>Cant. Máx.<input name="configuredCapacity" type="number" min="1" step="1" required defaultValue={editing?.capacityMode === 'ENROLLMENTS' ? editing.configuredCapacity : 1}/></label>}
