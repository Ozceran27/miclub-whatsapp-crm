...
test('conserva la foto original al cancelar y revoca previews',async()=>{const source=await readFile(new URL('./WorkerDraftList.tsx',import.meta.url),'utf8');assert.match(source,/const revokePreview=.*URL\.revokeObjectURL/);assert.match(source,/useEffect\(\(\)=>\(\)=>\{if\(previewRef\.current\)URL\.revokeObjectURL/);assert.match(source,/id!==editing\?\.photoFileId/);assert.match(source,/const cancel=\(\)=>\{discardTemporaryPhoto\(photoFileId\)/);assert.match(source,/if\(editing\?\.photoFileId&&editing\.photoFileId!==photoFileId\)/);});
