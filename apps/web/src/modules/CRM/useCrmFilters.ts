...
export const DEFAULT_SORT_BY: SortBy = 'lastPaymentAt';
...
export const SORT_LABEL_BY_VALUE = SORT_OPTIONS.reduce((acc, option) => ({ ...acc, [option.value]: option.label }), {} as Record<SortBy, string>);
const isEmptySortValue = (value: SortValue) => value === null || value === undefined || (typeof value === 'string' && value.trim() === '');
const compareText = (a: SortValue, b: SortValue) => {
  const aEmpty = isEmptySortValue(a); const bEmpty = isEmptySortValue(b);
  if (aEmpty || bEmpty) return aEmpty === bEmpty ? 0 : aEmpty ? 1 : -1;
  return String(a).localeCompare(String(b), 'es-AR', { sensitivity: 'base' });
};
const compareNumber = (a: SortValue, b: SortValue) => {
  const aNumber = typeof a === 'number' ? a : Number(a); const bNumber = typeof b === 'number' ? b : Number(b);
  const aEmpty = isEmptySortValue(a) || Number.isNaN(aNumber); const bEmpty = isEmptySortValue(b) || Number.isNaN(bNumber);
  if (aEmpty || bEmpty) return aEmpty === bEmpty ? 0 : aEmpty ? 1 : -1;
  return aNumber - bNumber;
};
const compareDate = (a: SortValue, b: SortValue) => {
  const aTime = typeof a === 'string' || typeof a === 'number' ? Date.parse(String(a)) : NaN; const bTime = typeof b === 'string' || typeof b === 'number' ? Date.parse(String(b)) : NaN;
  const aEmpty = isEmptySortValue(a) || Number.isNaN(aTime); const bEmpty = isEmptySortValue(b) || Number.isNaN(bTime);
  if (aEmpty || bEmpty) return aEmpty === bEmpty ? 0 : aEmpty ? 1 : -1;
  return aTime - bTime;
};
...
const compareMembers = (a: Member, b: Member, sortBy: SortBy, sortDirection: SortDirection, contactedByMemberId: ContactedRecentResponse['byMemberId']) => {
  const aValue = getSortValue(a, sortBy, contactedByMemberId[a.id]); const bValue = getSortValue(b, sortBy, contactedByMemberId[b.id]);
  ...
  if (baseComparison === 0) return compareText(`${a.apellido} ${a.nombre}`, `${b.apellido} ${b.nombre}`);
...
export const useCrmFilters = (members: Member[], debtors: Member[], contactedRecent: ContactedRecentResponse) => {
  ...
  const changeSort = (nextSortBy: SortBy) => setSortBy((currentSortBy) => { if (currentSortBy === nextSortBy) { setSortDirection((currentDirection) => (currentDirection === 'asc' ? 'desc' : 'asc')); return currentSortBy; } setSortDirection(DEFAULT_SORT_DIRECTION); return nextSortBy; });
  ...
  const allActivities = Array.from(new Set(members.map((d) => d.actividad).filter((activity): activity is string => Boolean(activity))));
