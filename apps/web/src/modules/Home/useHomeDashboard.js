...
const normalizeText = (value) => (value ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().replace(/\s+/g, ' ').toLowerCase();
const formatDateTime = (value) => {
    if (!value)
        return 'Sin sincronización registrada';
    return new Date(value).toLocaleString('es-AR', { dateStyle: 'short', timeStyle: 'short' });
};
const normalizeStatus = (status) => normalizeText(status).replace(/[-–—_/]+/g, ' ').replace(/[^a-z0-9ñ\s]/g, '').replace(/\s+/g, ' ').trim();
...
const getStatusBucket = (member) => getStatusBucketFromRawStatus(normalizeStatus(String(member.estado ?? '')));
const isActiveMember = (member) => !['abandoned', 'cancelled'].includes(getStatusBucket(member) ?? '');
...
const getEnrollmentStatusBreakdown = (records, fallbackTotal) => {
    const breakdown = { total: records.length || fallbackTotal || 0, active: 0, current: 0, newEnrollment: 0, debtor: 0, abandoned: 0, cancelled: 0, others: 0 };
...
const mapSummaryStatusBreakdown = (statusBreakdown) => statusBreakdown ? {
...
export function useHomeDashboard() {
    ...
    const loadHome = async () => {
        ...
        setSectorError(null);
        try {
            const financePromise = fetch(apiUrl('/club-finance-summary'), { cache: 'no-store' }).then(async (response) => { if (!response.ok)
                throw new Error('No se pudo cargar el resumen financiero.'); return response.json(); }).catch((financeLoadError) => { setFinanceError(financeLoadError instanceof Error ? financeLoadError.message : 'Resumen financiero no disponible.'); return null; });
            const sectorPromise = fetch(apiUrl('/sector-operational-summary'), { cache: 'no-store' }).then(async (response) => { if (!response.ok)
                throw new Error('No se pudo cargar el resumen operativo por sector.'); return response.json(); }).catch((sectorLoadError) => { setSectorError(sectorLoadError instanceof Error ? sectorLoadError.message : 'Resumen operativo por sector no disponible.'); return null; });
    ...
    return useMemo(() => {
        ...
        const financialSummaryLines = [{ label: 'Liquidez', value: formatFinanceMoney(financeSummary?.liquidity), highlight: 'positiveCritical', iconBefore: '💰' }, { label: 'Caja', value: formatFinanceMoney(financeSummary?.cash) }, { label: 'Banco', value: formatFinanceMoney(financeSummary?.bank) }, { label: 'Dólares', value: formatUsd(financeSummary?.dollars) }];
        const operationalBalanceLines = [{ label: 'Cuotas a cobrar', value: isFiniteNumber(estimatedDebt) ? formatArPeso(estimatedDebt) : unavailableLabel }, { label: 'Saldos Pendientes', value: formatFinanceMoney(financeSummary?.pendingNetBalance) }, { label: 'Saldos a Pagar', value: formatPayableObligation(financeSummary?.saldosAPagar) }, { label: 'Saldo proyectado', value: formatFinanceMoney(financeSummary?.projectedBalance), highlight: 'positiveCritical', iconBefore: '📈' }];
        const incomeBySectorLines = financeSummary?.incomeBySector.length ? financeSummary.incomeBySector.map((item, index) => ({ id: `income-${item.name}`, label: item.name, value: formatArPeso(item.amount), highlight: index === 0 ? 'positiveCritical' : undefined, iconAfter: index === 0 ? '⭐' : undefined })) : [{ id: 'income-unavailable', label: 'Ingresos', value: unavailableLabel }];
        const expenseBySectorLines = financeSummary?.expenseBySector.length ? financeSummary.expenseBySector.map((item, index) => ({ id: `expense-${item.name}`, label: item.name, value: formatArPeso(item.amount), highlight: index === 0 ? 'negativeCritical' : undefined, iconAfter: index === 0 ? '🔻' : undefined })) : [{ id: 'expense-unavailable', label: 'Egresos', value: unavailableLabel }];
        ...
        const formatArDate = (value) => { if (!value)
            return '—'; const date = new Date(value); return Number.isNaN(date.getTime()) ? '—' : date.toLocaleDateString('es-AR'); };
