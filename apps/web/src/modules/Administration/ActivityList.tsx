...
export function ActivityList({ canViewFinancials }: { canViewFinancials: boolean }) {
  ...
  return (
    <section className="section-panel activity-list" aria-labelledby="activity-list-title" aria-busy={loading}>
      ...
      {activities.length > 0 && <div className="activity-list__table-wrap"><table className="activity-list__table"><thead><tr><th>Nombre</th><th>Sector</th><th>Instructor</th><th>Receptor económico</th><th>Estado</th><th>Inscriptos</th><th>Modalidad</th><th>Comisión / monto fijo</th><th>Color</th><th>Genera inscripciones</th><th>Métrica financiera anual</th><th>Acciones</th></tr></thead><tbody>{activities.map((activity) => {
        const financial = financialByActivity.get(activity.id);
        return <tr key={activity.id} className="activity-list__row" tabIndex={0} role="button" aria-label={`Ver detalle de ${activity.name}`} onClick={() => setSelectedActivityId(activity.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedActivityId(activity.id); } }}><td><strong><span aria-hidden="true">{getActivityVisual(activity.iconKey).glyph} </span>{activity.name}</strong></td><td>{activity.sectorName || 'Sin sector'}</td><td>{activity.instructorName || 'Sin asignar'}</td><td>{activity.responsiblePersonName || activity.instructorName || 'Sin asignar'}</td><td><span className="activity-list__status" data-active={activity.status === 'active'}>{displayStatus(activity.status)}</span></td><td>{integer.format(activity.currentEnrollments ?? 0)}</td><td>{activity.modality || 'Sin modalidad'}</td><td>{commission(activity)}</td><td><span className="activity-list__color"><i style={{ backgroundColor: activity.color || '#91a4c8' }} />{activity.color || 'Sin color'}</span></td><td>{activity.generatesEnrollments ? 'Sí' : 'No'}</td><td>{financial ? <FinancialMetric item={financial} /> : <span className="activity-list__unlinked">sin asociación financiera directa</span>}</td><td><div className="activity-list__actions" onClick={(event) => event.stopPropagation()}>{canEdit && <><button type="button" onClick={() => setEditingActivity(activity)}>Editar</button><button type="button" disabled={mutationId === activity.id} onClick={() => void mutate(activity, 'status')}>{activity.status === 'active' ? 'Desactivar' : 'Activar'}</button></>}{canArchive && <button className="activity-list__archive" type="button" disabled={mutationId === activity.id} onClick={() => void mutate(activity, 'archive')}>Archivar</button>}</div></td></tr>;
      })}</tbody></table></div>}
