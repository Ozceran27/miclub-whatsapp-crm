...
import { getAdministrationActivities, getAnnualActivityRanking } from '../../services/api/administrationApi';
...
const commission = (activity: AdministrationActivityDto) => activity.settlementMode === 'fixed'
  ...
  return (
    <section className="section-panel activity-list" aria-labelledby="activity-list-title" aria-busy={loading}>
      <div className="section-header activity-list__header">
        ...
        <button className="ghost-btn" type="button" onClick={() => void load()} disabled={loading}>{loading ? 'Actualizando…' : 'Actualizar actividades'}</button>
      </div>
      ...
      {activities.length > 0 && <div className="activity-list__table-wrap"><table className="activity-list__table"><thead><tr><th>Nombre</th><th>Sector</th><th>Responsable</th><th>Estado</th><th>Inscriptos</th><th>Modalidad</th><th>Comisión / monto fijo</th><th>Color</th><th>Genera inscripciones</th><th>Métrica financiera anual</th></tr></thead><tbody>{activities.map((activity) => {
        const financial = financialByActivity.get(activity.id);
        return <tr key={activity.id}><td><strong>{activity.name}</strong></td><td>{activity.sectorName || 'Sin sector'}</td><td>{activity.managerName || activity.instructorName || 'Sin asignar'}</td><td><span className="activity-list__status" data-active={activity.status === 'active'}>{displayStatus(activity.status)}</span></td><td>{integer.format(activity.currentEnrollments ?? 0)}</td><td>{activity.modality || 'Sin modalidad'}</td><td>{commission(activity)}</td><td><span className="activity-list__color"><i style={{ backgroundColor: activity.color || '#91a4c8' }} />{activity.color || 'Sin color'}</span></td><td>{activity.generatesEnrollments ? 'Sí' : 'No'}</td><td>{financial ? <FinancialMetric item={financial} /> : <span className="activity-list__unlinked">sin asociación financiera directa</span>}</td></tr>;
      })}</tbody></table></div>}
