...
import { activityStatusLabel, describeActivityTerms, formatActivityDate, formatActivityProfitability, formatFrequency, formatMoney } from './activityPresentation';
...
export function ActivityList({ canViewFinancials }: { canViewFinancials: boolean }) {
  ...
  return <section className="section-panel activity-list" aria-labelledby="activity-list-title" aria-busy={loading}>
    ...
    {activities.length>0&&<div className="activity-list__table-wrap"><table className="activity-list__table"><thead><tr><th>Actividad</th><th>Sector</th><th>Responsables</th><th>Estado</th><th>Inscripciones</th><th>Condición económica</th><th>Precios</th><th>Color</th>{canViewFinancials&&<th>Rentabilidad operativa anual</th>}<th><span className="sr-only">Abrir</span></th></tr></thead><tbody>{activities.map(activity=>{const visual=getActivityVisual(activity.iconKey);const color=activity.color||'#91A4C8';return <tr key={activity.id} className="activity-list__row" tabIndex={0} role="button" aria-label={`Ver detalle de ${activity.name}`} onClick={()=>setSelectedActivityId(activity.id)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();setSelectedActivityId(activity.id);}}}>
      <td><span className="activity-list__identity" style={{'--activity-color':color} as CSSProperties}><i aria-hidden="true">{visual.glyph}</i><span><strong>{activity.name}</strong><small>{activity.modality||'Sin modalidad operativa'}</small></span></span></td><td><strong>{activity.sectorName||'Sin sector'}</strong></td><td><span className="activity-list__people"><strong>{activity.responsibleEmployeeName||activity.instructorName||'Sin asignar'}</strong><small>Receptor: {activity.responsiblePersonName||activity.responsibleEmployeeName||'automático'}</small></span></td><td><span className="activity-list__status" data-active={activity.status==='active'}>{activityStatusLabel(activity.status)}</span></td><td><span className="activity-list__enrollments"><strong>{integer.format(activity.currentEnrollments??0)}</strong><small>{activity.generatesEnrollments?'Admite nuevas':'No admite nuevas'}</small></span></td><td><span className="activity-list__terms"><strong>{describeActivityTerms(activity)}</strong><small>{activity.termsEffectiveFrom?`Desde ${formatActivityDate(activity.termsEffectiveFrom)}`:'Sin vigencia'}</small></span></td><td><span className="activity-list__prices">{activity.pricingConfigured?<><strong>Inscripción {formatMoney(activity.enrollmentPrice??0,activity.priceCurrencyCode??activity.operatingCurrencyCode)}</strong><small>Cuota {formatMoney(activity.feePrice??0,activity.priceCurrencyCode??activity.operatingCurrencyCode)} · {formatFrequency(activity.feeFrequency)}</small></>:<strong>Sin configurar</strong>}</span></td><td><span className="activity-list__color" role="img" aria-label={`Color ${color.toUpperCase()}`} title={color.toUpperCase()}><i style={{backgroundColor:color}}/></span></td>{canViewFinancials&&<td><span className="activity-list__financial"><strong data-negative={(activity.annualOperatingProfitability??0)<0}>{formatActivityProfitability(activity)}</strong><small>{activity.annualOperatingMovements??0} movimientos · {activity.annualOperatingProfitabilityYear??''}</small></span></td>}<td><span className="activity-list__arrow" aria-hidden="true">›</span></td>
    </tr>;})}</tbody></table></div>}
