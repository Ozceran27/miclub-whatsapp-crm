...
export function MovementCreateModal({open,onClose,onCreated}:Props){
 ...
 return <div className="sector-modal__backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className="sector-modal movement-form-modal" role="dialog" aria-modal="true" aria-labelledby="movement-form-title">
  <header className="sector-modal__header"><div><p className="eyebrow">Tesorería</p><h2 id="movement-form-title">Cargar movimiento</h2><p>Registrá un ingreso o egreso con clasificación canónica.</p></div><button className="sector-modal__close" type="button" onClick={onClose} aria-label="Cerrar">×</button></header>
  {!catalogs&&!error?<p>Cargando catálogos…</p>:<form className="movement-form" onSubmit={submit}>
   ...
   <Field label="Cuenta financiera"><select name="accountId" required defaultValue=""><option value="" disabled>Seleccionar…</option>{catalogs?.accounts.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field><Field label="Medio de pago"><select name="paymentMethodId" required defaultValue=""><option value="" disabled>Seleccionar…</option>{catalogs?.paymentMethods.filter(x=>x.isActive!==false).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field>
