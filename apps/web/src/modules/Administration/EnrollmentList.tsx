...
export function EnrollmentList() {
  ...
  const generate=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const f=new FormData(event.currentTarget);setGenerateBusy(true);setGenerateError('');void apiJson('/api/finance/receivables/generate',{method:'POST',headers:{'idempotency-key':crypto.randomUUID()},body:JSON.stringify({month:f.get('month'),reason:f.get('reason')})}).then(()=>void load()).catch(cause=>setGenerateError(cause instanceof Error?cause.message:'No se pudieron generar las cuotas.')).finally(()=>setGenerateBusy(false));};
  return <PaginatedList id="enrollment-list" eyebrow="Inscripciones" title="Inscripciones del club" description="Consulta paginada; sólo se descargan 20 registros por vez." searchLabel="Persona, actividad o sector" search={search} status={status} statusLabel="Estado" statusOptions={statusOptions} loading={loading} error={error} total={total} page={page} pageSize={PAGE_SIZE} emptyMessage="No hay inscripciones para estos filtros." onSearchChange={setSearch} onStatusChange={setStatus} onFilter={(event) => { event.preventDefault(); setPage(1); setFilters({ search, status }); }} onPageChange={setPage} onRetry={() => void load()}>
    {permissions.includes(PERMISSIONS.ENROLLMENTS_CREATE)&&<details><summary>Generar cuotas del mes</summary><form className="finance-form" onSubmit={generate}><label>Mes<input name="month" type="month" defaultValue={new Date().toLocaleDateString('en-CA',{timeZone:'America/Argentina/Buenos_Aires'}).slice(0,7)} required/></label><label>Motivo<input name="reason" required/></label><button disabled={generateBusy}>{generateBusy?'Generando…':'Generar cuotas'}</button></form>{generateError&&<p role="alert">{generateError}</p>}</details>}
    ...
    {selectedEnrollment && <EnrollmentDetailModal enrollment={selectedEnrollment} onClose={() => setSelectedEnrollment(null)} onChanged={() => {setSelectedEnrollment(null);void load();}} />}
  </PaginatedList>;
}
