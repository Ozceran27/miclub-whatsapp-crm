import type { AdministrationSectorDto, AdministrationWorkerBalance, AdministrationWorkerDto, AdministrationWorkerMutationDto } from '@miclub/shared';
...
type Props = { worker?: AdministrationWorkerDto; balance?: AdministrationWorkerBalance; onClose: () => void; onSave?: (input: AdministrationWorkerMutationDto) => Promise<void>; onArchive?: () => Promise<void>; onReload?: () => Promise<void> };
...
function WorkerBalanceDetails({ balance }: { balance: AdministrationWorkerBalance }) {
  return <section className="worker-modal__balances"><h3>Saldo a liquidar</h3>{balance.status === 'INCOMPLETE' ? <p>No disponible: el circuito financiero informó datos incompletos.</p> : balance.amounts.length ? <ul className="sector-modal__items">{balance.amounts.map(item => <li key={item.currencyCode}><strong>{item.amount < 0 ? 'A cobrar ' : 'A pagar '}{new Intl.NumberFormat('es-AR',{style:'currency',currency:item.currencyCode,maximumFractionDigits:2}).format(Math.abs(item.amount))}</strong><span>{item.pendingReview ? 'Pendiente de revisión' : item.currencyCode}</span></li>)}</ul> : <p>Sin saldo devengado.</p>}</section>;
}

export function WorkerDetailModal({ worker, balance, onClose, onSave, onArchive, onReload }: Props) {
  const titleId = useId();
  ...
  >
    <form id="administration-worker-form" onSubmit={event => { void submit(event); }} className="draft-form">
      {worker && balance && <WorkerBalanceDetails balance={balance} />}
  ...
  return <div className="sector-modal__backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="sector-modal worker-modal" role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} ref={dialogRef} tabIndex={-1}>
      ...
      {balance && <WorkerBalanceDetails balance={balance} />}
