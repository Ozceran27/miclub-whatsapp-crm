...
export function PaginatedList({ id, eyebrow, title, description, searchLabel, search, status, statusLabel, statusOptions, loading, error, total, page, pageSize, children, actions, overlays, emptyMessage, onSearchChange, onStatusChange, onFilter, onPageChange, onRetry }: Props) {
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  return <section className="section-panel paginated-list" aria-labelledby={`${id}-title`} aria-busy={loading}>
    <div className="section-header"><div><p className="eyebrow">{eyebrow}</p><h3 id={`${id}-title`} tabIndex={-1}>{title}</h3><p>{description}</p></div><strong className="paginated-list__total">{error ? 'Total no disponible' : `${total.toLocaleString('es-AR')} en total`}</strong></div>
    {actions}
