...
import { archiveAdministrationWorker, createAdministrationWorker, getAdministrationWorkers, updateAdministrationWorker } from '../../services/api/administrationApi';
  ...
  return (
    <section className="section-panel worker-list" aria-labelledby="worker-list-title" aria-busy={loading}>
      <div className="section-header worker-list__header">
        ...
        <div><button className="primary-btn" type="button" onClick={() => setCreating(true)}>Nuevo trabajador</button> <button className="ghost-btn" type="button" onClick={() => void load()} disabled={loading}>{loading ? 'Actualizando…' : 'Actualizar equipo'}</button></div>
      </div>
      ...
      {creating && <WorkerDetailModal onClose={() => setCreating(false)} onSave={async input => { await createAdministrationWorker(input); await load(); }}/>}
      {selectedWorker && <WorkerDetailModal worker={selectedWorker} onClose={() => setSelectedWorker(null)} onSave={async input => { await updateAdministrationWorker(selectedWorker.id,input); await load(); }} onArchive={async()=>{await archiveAdministrationWorker(selectedWorker.id);await load();}} />}
    </section>
  );
}
