import { PERMISSIONS, type AdministrationWorkerDto, type AdministrationWorkersResponse } from '@miclub/shared';
...
const compensation = (worker: AdministrationWorkerDto) => worker.hasFixedCompensation && worker.fixedCompensationAmount != null && worker.currencyCode
  ? `${new Intl.NumberFormat('es-AR',{style:'currency',currency:worker.currencyCode}).format(worker.fixedCompensationAmount)} · ${worker.fixedCompensationFrequency ?? 'sin frecuencia'}`
  : 'Según actividades';
