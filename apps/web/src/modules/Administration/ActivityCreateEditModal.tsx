  ...
  return <div className="sector-modal__backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose(); }}>
    <div className="sector-modal activity-form-modal" role="dialog" aria-modal="true" aria-labelledby="activity-form-title" ref={dialogRef} tabIndex={-1}>
      ...
      {loadingCatalogs ? <p role="status">Cargando sectores, instructores e iconos…</p> : <form className="activity-form" onSubmit={(event) => void submit(event)}>
        ...
        <fieldset className="activity-form__terms"><legend>Condiciones de liquidación</legend><label><input type="radio" name="termsMode" checked={mode === 'FIXED'} onChange={() => setMode('FIXED')} /> Monto fijo para el club</label><label><input type="radio" name="termsMode" checked={mode === 'VARIABLE'} onChange={() => setMode('VARIABLE')} /> Porcentaje del club</label>{mode === 'FIXED' ? <> <label>Monto fijo<input name="fixedClubFee" type="number" min="0" step="0.01" required defaultValue={activity?.settlementFixedAmount ?? 0} /></label><label>Moneda<select name="currencyCode" defaultValue={activity?.currencyCode ?? 'ARS'}><option>ARS</option><option>USD</option><option>BRL</option><option>EUR</option></select></label><label>Frecuencia<select name="fixedFeeFrequency" defaultValue={activity?.fixedFeeFrequency ?? 'MONTHLY'}><option value="DAILY">Diaria</option><option value="WEEKLY">Semanal</option><option value="MONTHLY">Mensual</option><option value="YEARLY">Anual</option></select></label></> : <label>Porcentaje del club<input name="clubSharePercentage" type="number" min="0" max="100" step="0.01" required defaultValue={activity?.clubSharePercentage ?? 0} /></label>}<label>Vigente desde<input name="effectiveFrom" type="date" required defaultValue={activity?.termsEffectiveFrom ?? new Date().toISOString().slice(0, 10)} /></label></fieldset>
