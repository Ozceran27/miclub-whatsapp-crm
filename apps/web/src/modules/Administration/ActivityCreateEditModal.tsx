...
export function ActivityCreateEditModal({ activity, onClose, onSaved }: Props) {
  ...
  const priceEffectiveDefault=latestPrice?nextDay(latestPrice.effectiveFrom):today();
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if((!activity||editPrices) && (!Number.isSafeInteger(number('enrollmentPrice'))||number('enrollmentPrice')<0||!Number.isSafeInteger(number('feePrice'))||number('feePrice')<0)){setError('Los precios deben ser importes enteros no negativos.');return;}
    ...
    const input: AdministrationActivityMutation = {
      ...
      status: field('status') as 'active' | 'inactive', notes: field('notes').trim() || null,
       ...
       ...(!activity||editPrices?{pricing:{enrollmentPrice:number('enrollmentPrice'),feePrice:number('feePrice'),feeFrequency:field('feeFrequency') as 'DAILY'|'WEEKLY'|'MONTHLY'|'YEARLY',effectiveFrom:field('priceEffectiveFrom')}}:{}),
       schedules,
    };
  ...
  return <ConfigurationEditorModal size="large" eyebrow="Actividades" title={activity ? 'Editar actividad' : 'Agregar Nueva Actividad'} description="Definí la operación y versioná las condiciones económicas sin alterar su historia." busy={saving} onClose={onClose}
    ...
    {loadingCatalogs ? <p role="status">Cargando sectores, trabajadores y vigencias…</p> : <form id="administration-activity-form" className="draft-form activity-form" onSubmit={event=>void submit(event)}>
      ...
      {activity&&!editPrices&&<section className="activity-form__current-terms"><div><small>Precios vigentes</small><strong>Inscripción {activity.enrollmentPrice} · Cuota {activity.feePrice} / {activity.feeFrequency}</strong></div><button type="button" className="ghost-btn" onClick={()=>setEditPrices(true)}>Programar nuevos precios</button></section>}
      {editPrices&&<fieldset><legend>Precios para alumnos</legend>{activity&&!activity.pricingConfigured&&<p className="activity-form__history-note">Sin configurar. La actividad sigue operativa; definí sus precios para nuevas inscripciones.</p>}<div className="activity-prices__grid"><label>Inscripción<FormattedValueInput kind="money" currency={priceCurrency} name="enrollmentPrice" required defaultValue={latestPrice?.enrollmentPrice??activity?.enrollmentPrice??0}/></label><label>Cuota<FormattedValueInput kind="money" currency={priceCurrency} name="feePrice" required defaultValue={latestPrice?.feePrice??activity?.feePrice??0}/></label><label>Frecuencia<select name="feeFrequency" defaultValue={latestPrice?.feeFrequency??activity?.feeFrequency??'MONTHLY'}><option value="DAILY">Diaria</option><option value="WEEKLY">Semanal</option><option value="MONTHLY">Mensual</option><option value="YEARLY">Anual</option></select></label><label>Vigente desde<input type="date" name="priceEffectiveFrom" required min={latestPrice?priceEffectiveDefault:undefined} defaultValue={priceEffectiveDefault}/></label></div><small>Moneda base del club: {priceCurrency}. Los precios anteriores permanecen en la historia.</small>{activity?.pricingConfigured&&<button type="button" className="ghost-btn" onClick={()=>setEditPrices(false)}>Conservar precios actuales</button>}</fieldset>}
