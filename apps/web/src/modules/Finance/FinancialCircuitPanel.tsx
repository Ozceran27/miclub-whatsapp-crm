...
function FinancialCircuitContent({ summaryOnly = false }: { summaryOnly?: boolean }) {
  ...
  return <section className="section-panel finance-circuit" aria-label="Circuito financiero">
    ...
    {summaryOnly ? null : <>
      ...
      {tab==='reconciliation' && <div id="finance-panel-reconciliation" role="tabpanel" aria-labelledby="finance-tab-reconciliation"><div className="finance-overview-actions">{can(PERMISSIONS.FINANCE_READ)&&<button type="button" onClick={()=>openAdministrationArea('movement-list')}>Abrir Movimientos</button>}{can(PERMISSIONS.ENROLLMENTS_VIEW)&&<button type="button" onClick={()=>openAdministrationArea('enrollment-list')}>Abrir Inscripciones</button>}</div>{canReconcile&&<ReconciliationQueue clubId={clubId} mutate={mutate} busy={busy}/>}
      {wb && <>
