import { formatEconomyDate, getMovementAmountLabel, getMovementPersonLabel, getMovementTone } from './formatters';
      ...
      <div className="finance-lines finance-lines--compact">
        {movements.length === 0 ? <p className="empty-card-note">No hay movimientos recientes.</p> : movements.map((movement) => (
          <span className={`finance-metric-row finance-metric-row--highlight-${getMovementTone(movement) === 'negativeCritical' ? 'red' : 'green'}`} key={movement.id}>
            <strong className="finance-metric-row__label">{movement.concept || movement.category || 'Movimiento'} · {getMovementPersonLabel(movement)}<small> · {formatEconomyDate(movement.movementDate)}</small></strong>
