...
export const ADMINISTRATION_CAPABILITIES = {
  ...
  createMovement: [PERMISSIONS.FINANCE_READ, PERMISSIONS.SECTORS_VIEW, PERMISSIONS.ACTIVITIES_VIEW],
  createEnrollment: [PERMISSIONS.PEOPLE_READ, PERMISSIONS.ACTIVITIES_VIEW],
...
const operationFor: Partial<Record<AdministrationCapability, AuthorizationCapability>> = {
  createMovement: 'MOVEMENTS_CREATE',
  createEnrollment: 'ENROLLMENTS_CREATE',
};
export const hasAdministrationCapability = (permissions: readonly string[], capability: AdministrationCapability) =>
  ...
  && (!operationFor[capability] || hasAuthorizationCapability(permissions, operationFor[capability]));
