...
DO $$ DECLARE n integer; cid uuid; BEGIN
 SELECT count(*),min(id) INTO n,cid FROM miclub.clubs WHERE lower(name)=lower('miClub') OR lower(email)=lower('miclub.posadas@gmail.com');
...
SELECT (SELECT count(*) FROM miclub.movements) movements_count,
       (SELECT coalesce(sum(amount),0) FROM miclub.movements WHERE upper(type)='INGRESOS') ingresos,
       (SELECT coalesce(sum(amount),0) FROM miclub.movements WHERE upper(type)='EGRESOS') egresos,
...
DO $$ DECLARE t text; cid uuid; BEGIN
 ...
 FOREACH t IN ARRAY ARRAY[
  'sectors','activities','instructors','people','enrollments','movements','movement_categories',
  'payment_methods','payments','receivables','operational_balances','sector_settlements',
  'sheet_metric_snapshots','import_batches','import_errors','crm_message_templates',
...
DO $$ DECLARE b _before%rowtype; BEGIN
 ...
 OR b.ingresos<>(SELECT coalesce(sum(amount),0) FROM miclub.movements WHERE upper(type)='INGRESOS')
 OR b.egresos<>(SELECT coalesce(sum(amount),0) FROM miclub.movements WHERE upper(type)='EGRESOS')
