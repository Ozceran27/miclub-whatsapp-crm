...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM public.miclub_schema_migrations WHERE name='202609090002_financial_operating_circuit.sql' AND checksum<>'8ca524bc9c99e85e1118f7df828c98c8f12e572d29c6dabf32e8f7a0fcfdbfdd') THEN RAISE EXCEPTION 'Checksum incompatible'; END IF;
 IF NOT EXISTS(SELECT 1 FROM public.miclub_schema_migrations WHERE name='202609090002_financial_operating_circuit.sql') THEN
  EXECUTE $migration$-- DEC-017. Forward-only additive history; no inferred historical responsible.
