...
| `202608160001_commercial_plan_taxonomy.sql` | Confirma un catálogo de un plan gratuito y tres pagos, con clase comercial explícita y DEVELOPMENT reservado exclusivamente a testing, sin implementar cobros. | `7e2292eb5bb569de5597d363b1a51ec6b7d67c63470b9cbdd537bacc548b0257` | Después de `202608140007_plan_entitlements.sql`. |
...
| `202608180003_fix_rls_login_membership_resolution.sql` | Permite resolver el contexto tenant mínimo después de validar la contraseña pese a FORCE RLS y completa con FREE los clubes sin suscripción activa. | `2d05b5f421cdceb22f6bfd161643cc705c6035bc111277d59be0d002100e0913` | Después de `202608180002_restore_runtime_application_grants.sql` y `202608160001_commercial_plan_taxonomy.sql`. |
...
| `202608270003_activity_fee_frequencies.sql` | Añade frecuencias de inscripción y monto fijo, conservando el valor mensual histórico. | `6e52f6d23967e423d7e2b2d4446078fb785db012f0146b61fc1b643544a66885` | Después de `202608270002_employee_compensation_and_photos.sql`. |
...
| `202609050002_fix_opening_balance_financial_status.sql` | Corrige los movimientos de saldos iniciales para usar el estado financiero canónico pagado sin ampliar el enum. | `c475a1512c6ca4222cdc4b2ebb994a3f2d8520178c174f1aa9018bd053e6ff20` | Después de `202608210001_operational_currency_opening_balances.sql`. |
| `202609050003_classify_cmv_as_non_operational.sql` | Clasifica CMV como egreso no operativo y repara sus vínculos tenant sin alterar movimientos ni importes históricos. | `c63d9555e9d0b62ff93b07d46b048b8e2d4e7d2c7e70fa3650c82ff6637fdd3c` | Después de `202608210003_complete_product_category_catalog.sql`. |
| `202609080002_fix_opening_balance_sequences.sql` | Asigna secuencias tenant a saldos iniciales y protege sus reversiones explícitas sin permitir egresos negativos ordinarios. | `d8ed9e60cad1e4f0a96117da50993f0b927fabbe3b8378d3bd2f192b9bb85ce4` | Después de `202609050002_fix_opening_balance_financial_status.sql` y `202608130006_tenant_entity_sequences.sql`. |
| `202609080003_retire_required_legacy_worker_payment.sql` | Retira la obligatoriedad del campo laboral legacy y conserva las restricciones de remuneración canónica. | `280357a33c84e52a1b0665b8348951077d778fa8b55768f13ec214b29a7c03ea` | Después de `202608280003_compensation_and_activity_term_currencies.sql`. |
| `202609080004_allow_seven_onboarding_steps.sql` | Permite persistir el séptimo paso de finalización del onboarding. | `e7aec4cbf6ba38f5eb97efd027ed9e011536cdc4f01db92d2c1b644fcb690c85` | Sin dependencia operativa adicional. |
...
| `202609200001_required_system_sectors.sql` | Normaliza y garantiza Administración, Tesorería y Áreas Comunes como los tres sectores de sistema activos de cada club. | `432c12148fdcd6a65f7d96614507a09f73f81e776e96b9eef45a083b19e91ac0` | Después de `202609190001_administration_configuration.sql`. |
| `202609210001_worker_activity_responsibility.sql` | Normaliza roles laborales, protege sectores de empleados por tenant y permite que cualquier trabajador activo sea responsable operativo de actividades. | `57e10ac34afff6fe9bd410a02f14948f07cafe251924a30f6612ae57eb099dd7` | Después de `202609200001_required_system_sectors.sql`. |
| `202609210002_canonical_activity_responsible_guard.sql` | Alinea la guarda de actividades activas con el trabajador responsable canónico y retira la exigencia legacy de Instructor. | `b0a2bc281f9c17e3d04f907403532138289e148eb8b7ce6c5af30ba2e35857be` | Después de `202609210001_worker_activity_responsibility.sql`. |
| `202609220001_activity_pricing_and_schedules.sql` | Versiona precios al alumno, protege horarios por tenant y captura precios en inscripciones. | `9e1a81f0630500f116fda1408dd131e0596136274066cc7cc96b8a410fbacc66` | Después de `202609210002_canonical_activity_responsible_guard.sql`. |
| `202609220002_cancel_future_activity_prices.sql` | Conserva para auditoría las vigencias futuras canceladas y excluye su solapamiento operativo. | `54f0feba7d5cc6e4280c1636a68906f5f43a891b1fa040ed83c1ea252b91f1d2` | Después de `202609220001_activity_pricing_and_schedules.sql`. |
| `202609250001_bidirectional_movement_categories.sql` | Permite ingresos y egresos con cualquier categoría activa sin cambiar identificadores ni movimientos históricos. | `b16fe616acdb795ab6535e3e41c4c73041a28b69049903ac6ac908ad1555a44c` | Después de `202609220002_cancel_future_activity_prices.sql`. |
